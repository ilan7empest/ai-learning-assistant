...
const FlashcardSetCard = ({ flashcardSet }: Props) => {
  ...
  const reviewdCount = flashcardSet.cards.filter((card) => card.lastReviewed).length;
  const totalCards = flashcardSet.cards.length;
  const progressPercentage = totalCards > 0 ? Math.round((reviewdCount / totalCards) * 100) : 0;
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          {reviewdCount > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-lg">
              ...
              <span className="text-sm font-semibold text-emerald-700">{progressPercentage}% Reviewed</span>
            </div>
          )}
        </div>
        {/* Progress bar */}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between ">
              ...
              <span className="text-xs font-semibold text-slate-700">
                {reviewdCount}/{totalCards} Reviewed
              </span>
            </div>
            <div className="relative h-2 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="absolute inset-y-0 left-0 bg-linear-to-r from-emerald-500 to-teal-500 rounded-full transition-all duration-500 ease-in"
      ...
      <div className="mt-6 pt-4 border-t border-slate-100">
        ...
        >
          <span className="relative z-10 flex items-center justify-center gap-2">
            <Sparkles className="w-4 h-4 mr-1" strokeWidth={2.5} />
            Study Now
          </span>
