...
const navLinks = [
    { text: 'Dashboard', to: '/dashboard', icon: LayoutDashboard },
    { text: 'Documents', to: '/documents', icon: FileText },
    { text: 'Flashcards', to: '/flashcards', icon: BookOpen },
...
const Sidebar = ({ isSidebarOpen, toggleSidebar }: Props) => {
    ...
    return (
        <>
            ...
            <aside className={`fixed top-0 left-0 w-64 h-full bg-white/90 backdrop-blur-lg border-r border-slate-200/50 z-50 md:relative md:w-64 md:shrink-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                {/* Logo and clode button in mobile */}
                <div className="flex items-center justify-between h-16 px-5 border-b border-slate-200/60">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center justify-center w-9 h-9 rounded-xk bg-linear-to-br from-emerald-400 to-teal-500 shadow-md shadow-emerald-500/20">
                ...
                <div className="px-3 py-4 border-t border-slate-200/60">
                    <button
                        ...
                        className="group w-full flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl text-slate-700 hover:bg-red-50 hover:text-red-600 transition-all duration-200"
                    >
