...
const AIActions = () => {
  ...
  const handleExplainConcept = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error('Failed to explain concept. Please try again later.');
      console.error(error);
    } finally {
      setLoadingAction(null);
    }
  ...
  return (
    <>
      <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
        {/* Header */}
        <div className="px-6 py-5 border-b border-slate-200/60 bg-linear-tp-br from-slate-50/50 to-white/50">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Explain Concept */}
          <div className="group p-5 bg-linear-to-br from-slate-50/50 to-white rounded-xl border border-slate-200/60 hover:border-slate-300/60 hover:shadow-md transition-all duration-20">
            <form onSubmit={handleExplainConcept}>
              ...
              <p className="text-sm text-slate-600 leading-relaxed mb-4">
                Enter a concept or term from the document to get a clear explanation.
              </p>
              <div className="flex items-center gap-3">
                <input
                  type="text"
                ...
                <button
                  ...
                  className="shrink-0 h-11 px-5 bg-linear-to-r from-emerald-600 to-emerald-500 hover:from-emerald-600 hover:to-emerald-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
                >
