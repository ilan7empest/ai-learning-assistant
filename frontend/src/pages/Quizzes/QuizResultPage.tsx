...
const QuizResultPage = () => {
  ...
  if (!quizResult?.quiz || quizResult.results.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <p className="text-slate-600">Quiz results not available.</p>
        </div>
      </div>
    );
  }

  const { quiz, results: userResults } = quizResult;
  const score = quiz.score;
  const totalQuestions = quiz.totalQuestions;
  const correctAnswers = userResults.filter((res) => res.isCorrect).length;
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <PageHeader title={`Quiz: ${quiz.title} Results`} />
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-6 mb-6">
        <div className="text-center space-y-6">
          <div className="inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 shadow-lg shadow-emerald-500/25 mx-auto">
            ...
            >
              ...
              <div className="space-y-3 mb-4">
                {result.options.map((option, optIndex) => {
                  ...
                  return (
                    <div
                      className={`relative px-4 py-3 rouned-lg border-2 transition-all duration-200 ${isCorrectOption ? 'bg-emerald-50 border-emerald-300 shadow-lg shadow-emerald-500/10' : isWrongAnswer ? 'bg-rose-50 border-rose-300' : 'bg-slate-50 border-slate-200'}`}
                      key={optIndex}
                    >
      ...
      <div className="mt-8 flex justify-center">
        <Link to={`/documents/${quiz.document._id}`}>
          <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold rounded-xl text-sm shadow-lg shadow-emerald-500/20 transition-all duration-200 active:scale-95 overflow-hidden">
