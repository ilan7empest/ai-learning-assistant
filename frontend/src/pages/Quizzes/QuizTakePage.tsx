...
const QuizTakePage = () => {
  ...
  const handleSubmitQuiz = async () => {
    setSubmitting(true);
    try {
      const formatAnswers = Object.keys(selectedAnswers).map((questionId) => {
        const question = quiz?.questions.find((q) => q._id === questionId);
        const questionIndex = quiz?.questions.indexOf(question!) || 0;
      ...
      await quizService.submitQuiz(quizId!, formatAnswers);
  ...
  return (
    <div className="max-w-4xl mx-auto">
          ...
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Custom Radion buttom */}
                <div
                  className={`shrink-0 w-5 h-5 rounded-full border-2 transition-all duration-200 ${isSelected ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}
                >
                  {isSelected && (
      ...
      <div className="flex justify-between items-center gap-4">
        <Button variant="secondary" disabled={currentQuestionIndex === 0} onClick={handlePreviousQuestion}>
