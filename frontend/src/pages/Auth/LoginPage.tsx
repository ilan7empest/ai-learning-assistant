...
import { BrainCircuit, Mail, Lock, ArrowRight, Eye } from 'lucide-react'
...
const LoginPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const { data: { user, token } } = await authService.login(userData.email, userData.password)
            login(user, token)
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    onBlur={() => setFocusedField(null)}
                                />
                                <div className={`absolute inset-y-0 right-0 pr-4 flex items-center hover:cursor-pointer transition-colors duration-200 ${focusedField === 'password' ? 'text-emerald-500' : 'text-slate-400'}`}>
                                    <Eye className='w-5 h-5' strokeWidth={2} onMouseDown={() => { setShowPassword(!showPassword) }} onMouseUp={() => { setShowPassword(!showPassword) }} />
                                </div>
                            </div>
                        </div>
                        {/* Error Message */}
